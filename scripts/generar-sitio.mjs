...
console.log(`Páginas estáticas copiadas de ${ORIGEN} a ${DESTINO}.`);
...
if (existsSync(rutaHistorico)) {
  try {
    ...
    console.warn('El registro histórico no pudo interpretarse; se publica vacío.');
  }
}
writeFileSync(join(DESTINO, 'datos', 'historico.json'), JSON.stringify(historico, null, 2));
console.log(`Registro histórico publicado con ${historico.length} ejecuciones.`);
...
if (existsSync('resultados/resumen.json')) {
  ...
  console.log('Resumen de la última ejecución publicado.');
  ...
  console.log('Sin resumen de esta corrida: se reutiliza el último registrado en el histórico.');
} else {
  console.log('Sin resumen de ejecución disponible.');
}
...
<p class="entradilla">Esta publicación no incluye el reporte navegable de la suite funcional.
Ocurre cuando el sitio se publica sin haber ejecutado las pruebas, por ejemplo tras un cambio que
solo afecta a la documentación.</p>
<p><a href="../index.html">Volver al portal</a></p></main></body></html>`
  );
  console.log('Sin reporte funcional en esta corrida: se publica una página sustituta.');
}
